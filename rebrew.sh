...
param|1|action|action to perform: install
param|1|package|package to install with brew
" \
| grep -v '^#' \
| sort
}
...
main() {
    ...
    [[ $(uname -m) == "arm64" ]] || die "Should only be run on native terminal (ARM64), not via Rosetta 2"
    action=$(lower_case "$action")
    case $action in
    install )
        #TIP: use «$script_prefix install» to recursively install a packahe with «brew install --build-from-source»
        #TIP:> $script_prefix install ffmpeg
